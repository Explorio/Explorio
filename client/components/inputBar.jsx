...
const InputBar = () => {
    ...
    const placresVisitedDispatcher = (placesVisitedArray) => dispatch(updatePlacesVisitedActionCreator);
    const placesWantToVisitDispatcher = (placesWantToVisitArray) => dispatch(updatePlacesWantToVisitActionCreator);
  ...
  // function to get the coordinates of an inputed location 
    ...
    const handleLocationInputClick = async (placesVisitedDispatcher, placesWantToVisitDispatcher) => {   
       // check which radio button was clicked
        ...
        try {
            ...
            const config = {
                ...
                data: { username: username, userId: userId, location: formattedAddress, lat: lat, long: lng, visitedCountries: placesVisitedBoolean }
            }
    ...
    return (
        <div class='input-bar-container'>
            <form id='location-form'>
                <input type='text' id='location-input' name="location-input" class='form-control' />
                <label for="location-input">Enter Location</label>
                <br />
                <input type="radio" id="places-visited" name="places-radio-btn" value="places-have-been" />
                <br />
                <input type="radio" id="places-want-to-visit" name="places-radio-btn" value="places-want-to-visit"/>
                <br />
                <button type='submit' class='btn' onclick={handleLocationInputClick}>Submit</button>
            </form>
        </div >
    )
}
...
{/* // radio group must share the same name (the value of the name attribute) to be treated as a group
        // he value is not shown to the user, but is the value that is sent to the server on "submit" to identify which radio button that was selected */}
